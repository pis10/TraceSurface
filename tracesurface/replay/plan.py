...
def build_replay_plan(
    ...
    *,
    ...
    safety = safety or ReplaySafetyPolicy()
    third_party = third_party or ThirdPartyPolicy(DEFAULT_THIRD_PARTY_DOMAINS)
    requests: list[ReplayRequest] = []
    seen_keys: set[str] = set()

    for target in job.cdp_requests:
        for req in build_cdp_requests(
            target,
    ...
    for cand in job.candidates:
        if not cand.resolution_id or cand.grade == "runtime":
            continue
...
def build_cdp_requests(
    ...
    *,
    ...
    third_party = third_party or ThirdPartyPolicy(DEFAULT_THIRD_PARTY_DOMAINS)
    if third_party.is_third_party(url, target_url):
        return ()
...
def build_requests(
    ...
    *,
    ...
    url = fill_path(url_raw, path_fill=path_fill)
